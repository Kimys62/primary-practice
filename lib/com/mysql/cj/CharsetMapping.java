...
public class CharsetMapping {
    ...
    protected static String getStaticMysqlCharsetForJavaEncoding(String javaEncoding, ServerVersion version) {
        ...
        if (mysqlCharsets != null) {
            if (version == null) {
                return mysqlCharsets.get((int)0).charsetName;
            }
            MysqlCharset currentChoice = null;
            for (MysqlCharset charset : mysqlCharsets) {
                if (!charset.isOkayForVersion(version) || currentChoice != null && currentChoice.minimumVersion.compareTo(charset.minimumVersion) >= 0 && (currentChoice.priority >= charset.priority || currentChoice.minimumVersion.compareTo(charset.minimumVersion) != 0)) continue;
    ...
    protected static int getStaticCollationIndexForJavaEncoding(String javaEncoding, ServerVersion version) {
        String charsetName = CharsetMapping.getStaticMysqlCharsetForJavaEncoding(javaEncoding, version);
        return CharsetMapping.getStaticCollationIndexForMysqlCharsetName(charsetName);
    }
    ...
    public static String getStaticCollationNameForCollationIndex(Integer collationIndex) {
        if (collationIndex != null && collationIndex > 0 && collationIndex < 1024) {
    ...
    protected static String getStaticJavaEncodingForMysqlCharset(String mysqlCharsetName, String fallbackJavaEncoding) {
        MysqlCharset cs = CharsetMapping.getStaticMysqlCharsetByName(mysqlCharsetName);
    ...
    protected static String getStaticJavaEncodingForMysqlCharset(String mysqlCharsetName) {
        return CharsetMapping.getStaticJavaEncodingForMysqlCharset(mysqlCharsetName, null);
    }
    ...
    public static String getStaticJavaEncodingForCollationIndex(Integer collationIndex) {
        return CharsetMapping.getStaticJavaEncodingForCollationIndex(collationIndex, null);
    }
    ...
    protected static int getStaticMblen(String charsetName) {
        MysqlCharset cs;
        if (charsetName != null && (cs = CharsetMapping.getStaticMysqlCharsetByName(charsetName)) != null) {
            return cs.mblen;
        }
        return 0;
    }
    ...
    static {
        ...
        TreeMap<Integer, MysqlCharset> collationIndexToCharset = new TreeMap<Integer, MysqlCharset>();
        ...
        TreeMap<String, Integer> collationNameToCollationIndexMap = new TreeMap<String, Integer>();
        HashSet<Integer> impermissibleIndexes = new HashSet<Integer>();
        for (int i = 1; i < 1024; ++i) {
            ...
            if (!charsetName.equals(MYSQL_CHARSET_NAME_ucs2) && !charsetName.equals(MYSQL_CHARSET_NAME_utf16) && !charsetName.equals(MYSQL_CHARSET_NAME_utf16le) && !charsetName.equals(MYSQL_CHARSET_NAME_utf32)) continue;
