...
RuntimeProperty.RuntimePropertyListener {
    ...
    public final <T extends Resultset> T sendQueryString(Query callingQuery, String query, String characterEncoding, int maxRows, boolean streamResults, ColumnDefinition cachedMetadata, ProtocolEntityFactory<T, NativePacketPayload> resultSetFactory) throws IOException {
        ...
        if (supportsQueryAttributes) {
            if (queryAttributes != null && queryAttributes.getCount() > 0) {
                ...
                for (int i = 0; i < queryAttributes.getCount(); ++i) {
                    if (!queryAttributes.getAttributeValue(i).isNull()) continue;
                    int n = i >>> 3;
                ...
                if (this.logSlowQueries) {
                    if (queryWasSlow) {
                        ...
                        if (this.propertySet.getBooleanProperty(PropertyKey.explainSlowQueries).getValue().booleanValue()) {
                            if (oldPacketPosition - queryPosition < 0x100000) {
    ...
    public final <T> T readServerStatusForResultSets(NativePacketPayload rowPacket, boolean saveOldStatus) {
        ...
        if (rowPacket.isEOFPacket()) {
            ...
            OkPacket ok;
            result = ok = OkPacket.parse(rowPacket, this.serverSession.getCharsetSettings().getErrorMessageEncoding());
    ...
    public SQLWarning convertShowWarningsToSQLWarnings(boolean forTruncationOnly) {
        ...
        try (ResultsetRows rows = null;){
            ...
            Resultset warnRs = this.readAllResults(-1, this.warningCount > 99, resultPacket, false, null, new ResultsetFactory(Resultset.Type.FORWARD_ONLY, Resultset.Concurrency.READ_ONLY));
    ...
    public void configureTimeZone() {
        ...
        if (this.getPropertySet().getBooleanProperty(PropertyKey.forceConnectionTimeZoneToSession).getValue().booleanValue()) {
            ...
            query.append("'");
            this.sendCommand(this.getCommandBuilder().buildComQuery(null, query.toString()), false, 0);
        }
    }
