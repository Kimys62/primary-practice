...
implements java.sql.CallableStatement {
        ...
        try {
            ...
            synchronized (object) {
                ...
                if (this.paramInfo != null && q.getParameterCount() != parameterCountFromMetaData) {
                    ...
                    if (startPos != -1 && (parenOpenPos = q.getOriginalSql().indexOf(40, startPos + 4)) != -1 && (parenClosePos = StringUtils.indexOfIgnoreCase(parenOpenPos, q.getOriginalSql(), ")", "'", "'", SearchMode.__FULL)) != -1) {
    ...
    protected boolean checkReadOnlySafeStatement() throws SQLException {
        if (ParseInfo.isReadOnlySafeQuery(((PreparedQuery)this.query).getOriginalSql(), this.session.getServerSession().isNoBackslashEscapesSet())) {
            ...
            if (StringUtils.startsWithIgnoreCaseAndWs(sql, "CALL", statementKeywordPos = ParseInfo.indexOfStatementKeyword(sql, this.session.getServerSession().isNoBackslashEscapesSet())) || StringUtils.startsWithIgnoreCaseAndWs(sql, "SELECT", statementKeywordPos)) {
                if (!this.connection.isReadOnly()) {
                    return true;
                }
                return this.checkReadOnlyProcedure();
            }
            return true;
        }
