    ...
    ParameterBindingsImpl(PreparedQuery<?> query, Session session, ResultSetFactory resultSetFactory) throws SQLException {
        ...
        for (int i = 0; i < paramCount; ++i) {
            ...
            switch (this.queryBindings.getBindValues()[i].getMysqlType()) {
                ...
                default: {
                    try {
                        charsetIndex = session.getServerSession().getCharsetSettings().getCollationIndexForJavaEncoding(this.propertySet.getStringProperty(PropertyKey.characterEncoding).getValue(), session.getServerSession().getServerVersion());
                        break;
                    }
